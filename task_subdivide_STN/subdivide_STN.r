...
mask_STN <-
  nat::read.nrrd(
    file = "/run/media/admin/LeGAO2/project_mouse_cortex/analysis/task_subdivide_STN/structure_470_STN_50um_ccf2017.nrrd",
    ReadByteAsRaw = FALSE)

dim(mask_STN)

# remove right hemisphere
mask_STN[,,115:228] <- 0

index_voxel <- which(mask_STN == 1)

label_voxel <- 1:length(index_voxel)

anno_STN <- mask_STN

anno_STN[index_voxel] <- label_voxel

nat::write.nrrd(
  x = anno_STN,
  file = "/run/media/admin/LeGAO2/project_mouse_cortex/analysis/task_subdivide_STN/anno_STN.nrrd",
  enc = "gzip",
  dtype = "ushort")
...
## 2. obtain neurons ----
index_PT_STN <-
  intersect(which(shared_proj_Combined[,snp::getStructureFromAcronym("l_STN")] > 100),
            df_All_index_AxonType$Index[df_All_index_AxonType$AxonType == "PT"])

saveRDS(
  object = index_PT_STN,
  file = "/run/media/admin/LeGAO2/project_mouse_cortex/analysis/task_subdivide_STN/data_reproduce/index_PT_STN.rds",
  version = 3,
  compress = "gzip")

## 3. calculate axon lengths in the cubes ----
proj_cube_STN <- snp::snp_calculateProjections(
  PathOfNeuron = shared_SWCPath_axon_allen[index_PT_STN],
  Hemisphere = info_tmp$Hemisphere[index_PT_STN],
...
saveRDS(
  object = proj_cube_STN,
  file = "/run/media/admin/LeGAO2/project_mouse_cortex/analysis/task_subdivide_STN/data_reproduce/proj_cube_STN.rds",
  version = 3,
  compress = "gzip")
...
index_cube_valid <- which(colSums(proj_cube_STN) > 0)

proj_cube_STN_valid <- proj_cube_STN[,index_cube_valid]


# 4. clustering ----
ff_similarity <-
  snp::snp_bigcorPar(
    x = proj_cube_STN_valid,
    size = 400,
    verbose = TRUE,
    ncore = 4,
    method = "spearman"
...
saveRDS(
  ...
  file = "/run/media/admin/LeGAO2/project_mouse_cortex/analysis/task_subdivide_STN/data_reproduce/matrix_similarity.rds",
  version = 3,
  compress = "gzip")
...
saveRDS(
  object = h,
  file = "/run/media/admin/LeGAO2/project_mouse_cortex/analysis/task_subdivide_STN/data_reproduce/h.rds",
  version = 3,
  compress = "gzip")
...
saveRDS(
  ...
  file = "/run/media/admin/LeGAO2/project_mouse_cortex/analysis/task_subdivide_STN/data_reproduce/label_cluster_real.rds",
  version = 3,
  compress = "gzip")


##
cluster_STN <- mask_STN
cluster_STN[index_voxel[index_cube_valid]] <- label_cluster_real


nat::write.nrrd(
  x = cluster_STN,
  file = "/run/media/admin/LeGAO2/project_mouse_cortex/analysis/task_subdivide_STN/cluster_STN_K5.nrrd",
  enc = "gzip",
  dtype = "byte")
...
ht <-
  Heatmap(
    matrix = matrix_similarity_ordered[seq(1, nrow(matrix_similarity), 1),
                                       seq(1, nrow(matrix_similarity), 1)],
    name = "Spearman's r",
    #col = col_fun,
...
for (iDomain in 1:NumCluster) {
  ...
  proj_domain[, iDomain] =
    rowSums(proj_cube_STN_valid[, which(label_cluster_real == iDomain)]) / volume_domain[iDomain, 1]
}
...
filename <- paste0("/tmp/STN_K5.svg")
#
U <- shared_soma_XYZUV_CC$U[index_PT_STN]
V <- shared_soma_XYZUV_CC$V[index_PT_STN]
Color <- "red"
...
df_neuron_domain <-
  data.frame(SourceRegion = info_tmp$Region[index_PT_STN], proj_domain)
